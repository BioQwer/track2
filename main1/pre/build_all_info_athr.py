...
def gen_athr_basic_info(author_f_name):
    ...
    reader = csv.reader(open(author_f_name, 'r', encoding='utf-8'))
    header = reader.next()
    for aid, aname, aff in reader:
        aid_post = int(aid)
...
def gen_paper_basic_info(paper_f_name):
    ...
    reader = csv.reader(open(paper_f_name, 'r',encoding='utf-8'))
    header = reader.next()
    for p_id, p_ttl, p_yr, p_cf_id, p_jr_id, p_keys in reader:
        p_id_post = int(p_id)
...
def main():
    if len(sys.argv) < 5:
        print('Author.csv, Paper.csv, PaperAuthor.csv, CN_table, dump_file')
        exit(0)
...
if __name__ == '__main__':
    # gen_paper_basic_info("../data/Paper.csv") # Testing only
    main()
